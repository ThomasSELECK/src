...
def create_dt(is_train = True, nrows = None, first_day = 1200):
    prices = pd.read_csv(SELL_PRICES_PATH_str, dtype = PRICE_DTYPES)
    for col, col_dtype in PRICE_DTYPES.items():
        if col_dtype == "category":
            prices[col] = prices[col].cat.codes.astype("int16")
            prices[col] -= prices[col].min()
            
    cal = pd.read_csv(CALENDAR_PATH_str, dtype = CAL_DTYPES)
    ...
    dt = pd.read_csv(SALES_TRAIN_PATH_str, nrows = nrows, usecols = catcols + numcols, dtype = dtype)
...
if __name__ == "__main__":
    ...
    print("df.shape (cell 14):", df.shape)
    ...
    sub.to_csv(PREDICTIONS_DIRECTORY_PATH_str + "draft_script_submission_2.csv",index=False)
